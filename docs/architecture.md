...
responses with an abortable delay. Retry state is surfaced in the TUI. Each
SSE event and the reconstructed tool arguments have explicit size limits;
overflow cancels the reader and ends the turn with a descriptive error.
