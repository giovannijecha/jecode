...
pub(crate) fn definitions_for(shell: &crate::command::Shell) -> Vec<Tool> {
    let mut tools: Vec<_> = [
        ...
        ("read_file", "Read UTF-8 text from an allowed local file, maximum file size 1 MiB. start_line is 1-based (default 1); max_lines defaults to 200 (1..400). Returns at most 8 KiB of complete lines with next_line when more remains. Very long individual lines fail. Excluded paths, links and binary files cannot be read.",
        ...
        ("create_file", "Propose creation of one absent UTF-8 text file in an existing allowed local directory. Required path and content, at most 32 KiB. Shows the complete diff and waits for explicit user approval. Never overwrites an existing entry or creates parent directories. A denied proposal must not be repeated without a new user request. No command execution.",
