...
export type Completion = { prefix: string; index: number };
 ...
 */
export function matches(text: string): readonly Command[] {
 ...
 */
export function pick(prefix: string, index: number): string | undefined {
  const list = matches(prefix);
  if (list.length === 0) return undefined;
  const at = ((index % list.length) + list.length) % list.length;
  return `/${(list[at] as Command).name}`;
}

export function activate(text: string): Completion | undefined {
  return matches(text).length === 0 ? undefined : { prefix: text, index: 0 };
}

export function move(completion: Completion, step: number): Completion {
  const count = matches(completion.prefix).length;
  if (count === 0) return completion;
  return {
    ...completion,
...
export function selected(completion: Completion): string | undefined {
  return pick(completion.prefix, completion.index);
}

/** Suggestions stay visible only while completion mode is active. */
export function options(completion: Completion | undefined): readonly Command[] {
