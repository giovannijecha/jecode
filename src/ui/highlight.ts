...
export type Role = "plain" | "comment" | "string" | "number" | "keyword";
export type Token = { text: string; role: Role };

type Family = "clike" | "python" | "shell" | "json" | "none";
...
const KEYWORDS: Record<Family, ReadonlySet<string>> = {
  clike: new Set([
...
const LINE_COMMENT: Record<Family, readonly string[]> = {
  clike: ["//"],
  python: ["#"],
  shell: ["#"],
  json: [],
...
export function family(lang: string): Family {
 ...
 */
export function highlight(lines: readonly string[], lang: string): Token[][] {
  const kind = family(lang);
  if (kind === "none") return lines.map((line) => [{ text: line, role: "plain" as const }]);
  ...
  return lines.map((line) => {
    ...
    const flush = (): void => {
      if (plain !== "") tokens.push({ text: plain, role: "plain" });
      plain = "";
    };
    const take = (text: string, role: Role): void => {
      flush();
    ...
    while (i < line.length) {
      ...
      if (word !== null && word.index === i) {
        if (keywords.has(word[0])) take(word[0], "keyword");
        else {
          plain += word[0];
          i += word[0].length;
        }
