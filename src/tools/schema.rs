...
pub(crate) fn definitions_for(shell: &crate::command::Shell, image: bool) -> Vec<Tool> {
    ...
    let mut tools: Vec<_> = [
        ...
        ("recall_receipts", "Retrieve an original, saved read-only tool result from this session without rereading its source or executing historical tools. Pass a supplied recall_address or next object unchanged: absolute canonical turn and step, original tool-call receipt index (default 0), UTF-8 byte offset (default 0), and optional expected_call_id. The identity guard rejects a coordinate pointing to another call. Response output_index and compaction record/fragment numbers are not receipt indices. One response contains at most 8 KiB of original bytes, bounded by the encoded output limit, and an explicit next position for more bytes or eligible receipts; skipped records retain their original indices. Follow next until the needed result is complete. If an aggregate batch returns paired not-admitted or not-executed results, consume the delivered pages and request the missing pages in a later call. Only recorded list_files, read_file and search_text results are available. This is an old observation; use a new read separately if current source state matters.",
