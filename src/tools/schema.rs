...
pub(crate) fn definitions_for(shell: &crate::command::Shell) -> Vec<Tool> {
    let mut tools: Vec<_> = [
        ...
        ("create_file", "Propose creation of one absent UTF-8 text file in an existing allowed local directory. Required path and content. The preview is bounded; when shortened, it reports omissions and an inspectable full diff path while approval waits. Never overwrites an existing entry or creates parent directories. A denied proposal must not be repeated without a new user request. No command execution.",
        r#"{"type":"object","properties":{"path":{"type":"string"},"content":{"type":"string"}},"required":["path","content"],"additionalProperties":false}"#),
        ("edit_file", "Propose one exact, unique text replacement in an ordinary allowed UTF-8 file, including files larger than 1 MiB. Read relevant lines first. Required path, old_text, and new_text. Empty old_text is only valid for an empty file. Preserve existing LF/CRLF. The preview is bounded; when shortened, it reports omissions and an inspectable full diff path while approval waits. Waits for explicit user approval and rejects stale files. Original is retained at the reported local recovery path. Do not retry a denied proposal without a new user request.",
