...
pub(crate) fn definitions_for(shell: &crate::command::Shell, image: bool) -> Vec<Tool> {
    ...
    let mut tools: Vec<_> = [
        ...
        ("index_receipts", "Locate saved workspace-read receipts covered by compaction without trusting the handoff. Start with {turn:0,step:0,receipt:0} and follow each next cursor. This lists original call names, arguments and guarded recall_address objects in canonical order; it does not return source contents or reread the workspace. Pass a returned recall_address unchanged to recall_receipts to read the exact saved content.",
        r#"{"type":"object","properties":{"mode":{"type":"string","enum":["index"]},"turn":{"type":"integer","minimum":0},"step":{"type":"integer","minimum":0},"receipt":{"type":"integer","minimum":0}},"required":["turn","step"],"additionalProperties":false}"#),
        ("recall_receipts", "Read the exact saved content of one historical list_files, read_file or search_text result without rerunning its workspace tool. Pass a guarded recall_address from index_receipts or the handoff unchanged, including expected_call_id; follow its next cursor for remaining UTF-8 bytes. Response output_index and compaction record/fragment numbers are not receipt indices. If a batch says not admitted, consume delivered pages and request missing pages later. A new workspace read is separate and may see changed source content.",
        r#"{"type":"object","properties":{"turn":{"type":"integer","minimum":0},"step":{"type":"integer","minimum":0},"receipt":{"type":"integer","minimum":0},"offset":{"type":"integer","minimum":0},"expected_call_id":{"type":"string","minLength":1,"maxLength":256}},"required":["turn","step"],"additionalProperties":false}"#),
