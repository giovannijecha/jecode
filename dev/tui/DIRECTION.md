...
| Composer and menus | The original full-width dock shell holds editing, completion, selectors, fields, and help. It does not inherit the narrower transcript column. Ribbon menus use a `●` selection marker, a subtle focus band, and aligned values without selected-choice descriptions. Clipped labels or values may use up to two stable overflow rows. Shared row and prompt renderers keep progress and cursor placement consistent. | `menu-workflow`, `menu-commands`, `menu-search`, `menu-settings`, `field`, `help` |
