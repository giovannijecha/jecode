...
| [`benchmarks/head-to-head/`](benchmarks/head-to-head/README.md) | Optional live Codex/Jecode comparison through real TUIs in WSL, with fixed tasks, external acceptance checks and private local evidence. |
| [`context/`](context/README.md) | Opt-in, bounded numeric diagnostics from real production TUI sessions. |
| [`experiments/`](experiments/README.md) | Isolated controller candidates, excluded from production until validated against the unchanged baseline. |
...
The live WSL comparison uses a private native Linux directory under `/var/tmp`
because tmpfs `/tmp` can disappear when the guest stops. Keep its immutable
inputs and local evidence while that comparison is active; remove temporary
account copies after execution, and retire the laboratory when the investigation
closes and the reviewed evidence has been retained.
