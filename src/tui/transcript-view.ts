...
import { runningTool } from "./components/tool-motion.ts";
...
export function transcriptRenderer(
  ...
  let layouts: Layout[] = [];
  const activeTools = new Set<Block>();
  ...
  return {
    ...
    invalidate(block) {
      ...
      if (runningTool(block)) activeTools.add(block);
      else activeTools.delete(block);
  ...
  function hasRunningEntry(layout: Layout, window: Window): boolean {
    for (const block of activeTools) {
      const index = layout.positions.get(block);
      if (index !== undefined && index >= window.first && index <= window.last &&
          runningTool(block) && block.expanded !== true) return true;
    }
    return false;
  }
}
