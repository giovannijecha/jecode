...
pub(crate) fn definitions_for(shell: &crate::command::Shell, image: bool) -> Vec<Tool> {
    ...
    let mut tools: Vec<_> = [
        ...
        ("recall_receipts", "Retrieve an original, saved read-only tool result from this session without rereading its source or executing historical tools. Use absolute canonical turn and step numbers from the handoff, then receipt index in call order (default 0) and UTF-8 byte offset (default 0). One response contains at most 8 KiB of original bytes, bounded by the encoded output limit, and an explicit next position for more bytes or eligible receipts; skipped records retain their original indices. Follow next until the needed result is complete. If an aggregate batch returns paired not-admitted or not-executed results, consume the delivered pages and request the missing pages in a later call. Only recorded list_files, read_file and search_text results are available. This is an old observation; use a new read separately if current source state matters.",
        r#"{"type":"object","properties":{"turn":{"type":"integer","minimum":0},"step":{"type":"integer","minimum":0},"receipt":{"type":"integer","minimum":0},"offset":{"type":"integer","minimum":0}},"required":["turn","step"],"additionalProperties":false}"#),
