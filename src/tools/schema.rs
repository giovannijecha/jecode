...
pub(crate) fn definitions_for(shell: &crate::command::Shell, image: bool) -> Vec<Tool> {
    ...
    let mut tools: Vec<_> = [
        ...
        ("recall_receipts", "Retrieve an original, saved read-only tool result from this session without rereading its source or executing historical tools. Use absolute canonical turn and step numbers from the handoff, then receipt index in call order (default 0) and UTF-8 byte offset (default 0). One response contains at most 8 KiB of original bytes, bounded by the encoded output limit, and an explicit next position for more bytes or receipts. Follow next until the needed result is complete. Only recorded list_files, read_file and search_text results are available. This is an old observation; use a new read separately if current source state matters.",
