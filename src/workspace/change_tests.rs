...
fn replacement_must_validate_composed_text_across_staging_threshold() {
    ...
    for padding in [10, MAX_FILE_BYTES - 15, MAX_FILE_BYTES - 14, 1_100_000] {
        ...
        let error = ws
            ...
            .err()
            .expect("a replacement leaving bare CR must be rejected before execution");
...
#[test]
fn original_401_line_create_is_accepted_with_bounded_preview() {
    let files = support::Fixture::new();
    let ws = Workspace::open(&files.0).unwrap();
    let cancelled = AtomicBool::new(false);
    ...
    assert!(preview.diff.contains("+ line 000 abc"));
...
#[test]
fn staged_large_deletion_has_bounded_diff_and_stale_or_cancelled_changes_do_not_publish() {
    let files = support::Fixture::new();
    let ws = Workspace::open(&files.0).unwrap();
    let cancelled = AtomicBool::new(false);
