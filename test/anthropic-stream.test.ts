...
test("keeps partial text but rejects a tool call truncated at max_tokens", async () => {
  const data = await assembleAnthropic(
    feed([
      ...
      { type: "content_block_stop", index: 0 },
      {
        type: "content_block_start",
        index: 1,
        content_block: { type: "tool_use", id: "partial-call", name: "write_file", input: {} },
      },
      {
        type: "content_block_delta",
        index: 1,
  ...
  assert.deepEqual(message.content, [
    ...
    { kind: "text", text: "[truncated: hit max_tokens — raise max output tokens in /settings]" },
  ]);
