//! SHA-512 family compression; FIPS 180-4 sections 4.1.3, 4.2.3 and 6.4.2.
// Numerical constants from the standard, not an external implementation.
const K: [u64; 80] = [
    0x428a2f98d728ae22,
    0x7137449123ef65cd,
    0xb5c0fbcfec4d3b2f,
...
pub(super) fn block(state: &mut [u64; 8], block: &[u8; 128]) {
    let mut words = [0_u64; 80];
    for (word, bytes) in words.iter_mut().zip(block.chunks_exact(8)) {
        *word = u64::from_be_bytes(bytes.try_into().expect("word"));
    }
    for i in 16..80 {
        let a = words[i - 15];
        let b = words[i - 2];
    ...
    for (word, constant) in words.iter().zip(K) {
